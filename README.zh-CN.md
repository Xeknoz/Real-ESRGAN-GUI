...
该脚本会自动编译需要更新的后端、准备共享 NCNN 模型、编译启动器、发布 WPF 主程序，并组装好 `artifacts/portable/<arch>/` 下可直接分发的目录。如果当前生成的 `artifacts/backend/<arch>/engine/realesrgan-ncnn-vulkan.exe` 已经与后端源码匹配，则会自动跳过后端编译；需要强制重编后端时使用 `.\scripts\build-all.ps1 -Clean -ForceBackend`。
