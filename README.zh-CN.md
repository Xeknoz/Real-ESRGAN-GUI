...
- 如果想免安装使用，下载单文件绿色版，例如 `Real-ESRGAN-GUI-Portable-x64.exe`。
...
Release 上传资产只包含安装包和 Enigma 单文件绿色版。除非显式传入 `-IncludePortableArchives`，否则不会生成 portable 目录压缩包。
...
artifacts\
  ...
  release-assets\           用于上传的安装包和单文件绿色版
