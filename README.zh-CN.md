...
日常使用很直接：选择图片文件夹，选择输出文件夹，选模型类型，然后开始处理。它适合处理照片、人像、动漫图片、插画和动画帧。
...
| 设置 | 实用建议 |
| --- | --- |
| 模型类型 | 真实照片选 `照片 / 人像`，绘画类图片选 `动漫 / 插画`，动画帧文件夹选 `动漫视频帧`。 |
| 放大倍数 | 不确定时保持 `模型默认`。只有需要固定 2x、3x 或 4x 输出时再手动选择。 |
| 保存格式 | 更在意保留质量选 `PNG`，想减小文件选 `JPG`，网页使用可选 `WebP`。选择 `动漫视频帧` 时，这里表示输出帧格式。 |
...
`动漫视频帧` 模型处理的是已经拆好的帧图片，不会直接读取 `mp4`、`avi` 或 `mov` 文件；请选择该模型类型前先把视频拆成帧。
