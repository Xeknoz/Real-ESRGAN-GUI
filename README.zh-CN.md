...
- 如果想免安装使用，下载 Release 里提供的便携压缩包或单文件绿色版，例如 `Real-ESRGAN-GUI-win-x64.zip` 或 `Real-ESRGAN-GUI-Portable-x64.exe`。
...
默认同时构建两个发布架构。脚本会先生成或复用 `artifacts\portable\<arch>\`，再把每个便携版目录打包为：
...
artifacts\
  ...
  release-assets\           用于上传的压缩包、安装包和单文件绿色版
