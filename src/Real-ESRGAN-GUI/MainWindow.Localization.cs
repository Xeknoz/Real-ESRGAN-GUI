namespace RealESRGAN_GUI
{
    public partial class MainWindow
    {
        ...
        private static string ChineseText(string key) => key switch
        {
            ...
            "GpuLabel" => "GPU 设备",
            "Tta" => "质量增强（较慢）",
            "Hint" => "请先根据图片内容选择模型类型，模型不匹配可能影响效果；其余设置保持默认即可。",
            "ModelPhoto" => "照片 / 人像",
            "ModelAnime" => "动漫 / 插画",
            "ModelVideo" => "动漫视频",
            "ScaleAuto" => "模型默认（{0}x）",
        ...
        private static string EnglishText(string key) => key switch
        {
            ...
            "Hint" => "Choose the model type first. A mismatched model can reduce quality; the other defaults are usually fine.",
            "ModelPhoto" => "Photo / portrait",
            "ModelAnime" => "Anime / illustration",
            "ModelVideo" => "Anime video",
